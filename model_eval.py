...
tf.app.flags.DEFINE_integer('num_examples', 516,
                            """Number of examples to run.""")
tf.app.flags.DEFINE_integer('batch_size', 2,
                            """Number of examples per batch.""")
tf.app.flags.DEFINE_string('data_txt', 'test_list.txt',
...
def _eval_once(saver, rmse_op, network):
  ...
  with tf.Session() as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
      ...
      return
    test_x, test_label = input_pipeline([FLAGS.data_txt], batch_size=FLAGS.batch_size, shape=[224, 224, 3], is_training=False)
    ...
    try:
      ...
      while step < num_iter and not coord.should_stop():
        ...
        rmse, pred = sess.run([rmse_op, network['pred']],feed_dict={network['x']: test_xs, network['y']: label, network['train']: False,
...
def evaluate(shape=[224, 224, 3]):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    with tf.device(FLAGS.device):
        deepid = deepID(input_shape=[None, 224, 224, 3], n_filters=[20, 40, 60, 80], 
    ...
    gt_truth = deepid['y']
    gt_truth = tf.reshape(gt_truth, (-1, 70, 2))
